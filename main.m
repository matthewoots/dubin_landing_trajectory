...
[path,nCi,nCf,flag] = dubinTimeBased(ip, lineup_point, ib, fb, minTurnRad, dist_int, flightHeight);
if ~flag
    return
end
...
for j = 1:3
    ...
    intv(j) = ceil(knot_span/timeint);
    
    [x(j+1,:), x(j+4,:), accel, x(1,:)] = getBSpline(order, wp_t, cp(j,:), intv(j), false);
end
